...
class PerformanceTester {
  ...
  private generateTransactionBatch(count: number) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const riskScore = Math.floor(Math.random() * 100) + 1; // Random risk score 1-100
      
      transactions.push({
        user_id: `user_${Math.floor(Math.random() * 100) + 1}`,
        device_id: `device_${Math.floor(Math.random() * 100) + 1}`,
        merchant_id: `merchant_${Math.floor(Math.random() * 100) + 1}`,
